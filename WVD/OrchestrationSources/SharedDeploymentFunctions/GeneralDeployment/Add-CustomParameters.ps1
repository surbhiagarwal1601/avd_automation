<#
.SYNOPSIS
Overwrite a given parameter.json with a version populated by the values in the provided valuemap
...
function Add-CustomParameters {
    ...
    process {
        ...
        foreach ($valueItem in $valueMap) {
            ...
            try {
                ...
                elseif ($valueItem.AddToArray) {
                    ...
                    Invoke-Expression ("`$paramFileContent.parameters.$path = @('{0}')" -f ($targetValue -join "`','"))
                }
                else {
        ...
        if ($PSCmdlet.ShouldProcess(("Paramter file [{0}]" -f (Split-Path $parameterFilePath -Leaf)), "Overwrite")) {
            ...
            Write-Verbose "Custom parameters added to file" -Verbose
        }
    }
