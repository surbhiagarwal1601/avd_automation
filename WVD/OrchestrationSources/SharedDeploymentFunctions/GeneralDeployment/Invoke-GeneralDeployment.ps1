  ...
  process {
    ...
    if ($Deployment.Outputs) {
      ...
      foreach ($OutputKey in $Deployment.Outputs.Keys) {
        $Type = $Deployment.Outputs[$OutputKey].Type
        Write-Verbose "Set Deployment Output: [$OutputKey] of type: [$Type] as pipeline variable." -Verbose
        If ($Type -eq 'Object') {
          [psobject]$Value = $Deployment.Outputs[$OutputKey].value.ToString() | ConvertFrom-Json
        }
        ElseIf ($Type -eq 'Array') {
          [array]$Value = $null
          $i = 0
          do {
            [array]$Value += $Deployment.Outputs.$OutputKey.Value[$i].Value  
            $i++          
          } while ($null -ne $Deployment.Outputs.$OutputKey.Value[$i].Value)
        }
        Else {
          $Value = $Deployment.Outputs.$OutputKey.Value
        }
        Write-Verbose "Set [$OutputKey] = $Value" -Verbose
        Write-Output "##vso[task.setvariable variable=$OutputKey;isOutput=true]$Value"
      }
