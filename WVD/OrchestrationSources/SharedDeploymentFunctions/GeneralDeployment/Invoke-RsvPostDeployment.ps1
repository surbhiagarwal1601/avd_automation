...
Each of those backup items needs a 'StorageAccountName' specified an optionally 'FileShareNames' as an array or comma separated list
...
.EXAMPLE
Invoke-RsvPostDeployment -orchestrationFunctionsPath $currentDir  -RecoveryServicesVaultName 'ProfilesBackupVault' -RecoveryServicesVaultResourceGroup 'resourcegroupname' -fileSharePolicyMaps $fileSharePolicyMaps

Add the items specified in the 'fileSharePolicyMaps' objects as backup items to the RSV 'ProfilesBackupVault' in resource group 'resourcegroupname'
#>
function Invoke-RsvPostDeployment {
    ...
    process {
        ...
        Write-Verbose "## 1 - Set protected items up ##" -Verbose
        Write-Verbose "################################" -Verbose
