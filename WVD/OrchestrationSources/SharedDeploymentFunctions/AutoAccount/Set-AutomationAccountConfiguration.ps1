<#
    ...
    Mandatory. Provide the name of the automation account name do you want create.

    .PARAMETER AutomationAccountRGName
    Mandatory. Resource group of the automation account

    .PARAMETER WorkspaceName
    Mandatory. Provide name of the log analytic workspace.

    .PARAMETER KeyVaultName
    Mandatory. The key vault to store the webhook uri data in. Required if other resources should use it.
...
function Set-AutomationAccountConfiguration {
    param(
        ...
        [pscustomobject[]] $RequiredAutoAccountModules = @(
            ...
            [pscustomobject]@{ ModuleName = 'Az.Storage' }
            [pscustomobject]@{ ModuleName = 'Az.Automation' }
            [pscustomobject]@{ ModuleName = 'Az.DesktopVirtualization' }
            [pscustomobject]@{ ModuleName = 'Az.OperationalInsights' }
            [pscustomobject]@{ ModuleName = 'Az.StorageTable' }
        ),
    ...
    begin {
        ...
        . "$PSScriptRoot/Set-LogAnalyticsConfiguration.ps1"
    }

    process {
        Write-Verbose "===========================" -Verbose
        Write-Verbose "== VALIDATE REQUIREMENTS ==" -Verbose

        Write-Verbose "Check requirement: Required role assignments" -Verbose
        ...
        Write-Verbose "All Requirement checks passed" -Verbose
    
        Write-Verbose "===========================" -Verbose
        Write-Verbose "== RUNBOOK CONFIGURATION ==" -Verbose
        ...
        if (-not $WebhookURIVar) {
            Write-Verbose "Create Webhook '$WebhookName'" -Verbose
            ...
            Write-Verbose "Automation Account Webhook is created with name '$WebhookName' and expiry date in 5 years"  -Verbose

            $WebhookUri = $Webhook.WebhookURI

            Write-Verbose "Create automation variable with webhook uri" -Verbose
            ...
            Write-Verbose "Webhook URI stored in Azure Automation Acccount variables" -Verbose
        }
        else {
            Write-Verbose "Webhook '$WebhookName' already existing for automation account '$AutomationAccountName'" -Verbose
        ...
        if ($KeyVaultName) {
            if (-not (Get-AzKeyVaultSecret -VaultName $KeyVaultName -Name "$WebhookName-Uri") -or -not((Get-AzKeyVaultSecret -VaultName $KeyVaultName -Name "$WebhookName-Uri").SecretValueText -eq $WebhookUri)) {
                Write-Verbose ("Store WebhookUri in keyvault '{0}' in secret '{1}'" -f $KeyVaultName, "$WebhookName-Uri") -Verbose
                ...
                Write-Verbose "WebUri already stored in key vault" -Verbose
            }
        }
    
        Write-Verbose "=========================" -Verbose
        Write-Verbose "== MODULE INSTALLATION ==" -Verbose
        foreach ($RequiredModule in $RequiredAutoAccountModules) {
            ...
            if ($null -eq $ImportedModule) {
                Write-Verbose ("Module '{0}' not in AutomationAccount. Adding it..." -f $RequiredModule.ModuleName) -Verbose
        ...
        if (-not [String]::IsNullOrEmpty($LAWorkspaceName)) {
            Write-Verbose "=========================" -Verbose
            Write-Verbose "== LOG ANALYTICS SETUP ==" -Verbose
            Set-LogAnalyticsConfiguration -LAWorkspaceName $LAWorkspaceName
        }
    }
