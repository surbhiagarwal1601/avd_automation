    ...
    process {
        ...
        while (-not $IsModuleImported -and $tryCount -lt $maxTries) { 
            ...
            if ($IsModule.ProvisioningState -eq "Succeeded") {
                ...
                Write-Verbose "Successfully $ModuleName module imported into Automation Account Modules..." -Verbose
            }
            else {
                Write-Verbose ("Waiting 10 seconds for module import of '{0}' into automation account [{1}|{2}]" -f $ModuleName, $tryCount, $maxTries) -Verbose
                $tryCount++;
                Start-Sleep 10 
            }
        }
