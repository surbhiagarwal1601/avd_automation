...
Set-Logger "C:\WindowsAzure\Logs\Plugins\PackageExecutionLogs\NPP" # inside "packageExecution_$scriptName_$date.log"

LogInfo("######################")
LogInfo("# Package: NotepadPP #")
LogInfo("######################")

LogInfo("######################")
LogInfo("# Download Notepad++ #")

$Url = "https://notepad-plus-plus.org/repository/7.x/7.7.1/npp.7.7.1.Installer.x64.exe"

$installBasePath = $PSScriptRoot
$installFilePath = Join-Path $installBasePath 'npp.7.8.8.Installer.x64.exe'
...
LogInfo("#####################")
LogInfo("# Install Notepad++ #")

$Switches = "/S"
LogInfo("Trigger installation of file '$installFilePath' with switches '$switches'")
$res = Invoke-Expression -Command "$installFilePath $Switches"
